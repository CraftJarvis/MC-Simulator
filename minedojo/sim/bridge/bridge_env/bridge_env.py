...
class BridgeEnv:
    ...
    def step(self, action_xmls: List[str]):
        ...
        # print(f"{len(action_xmls)} {len(self._instances)}")
        # if "use 0" not in action_xmls[0]:
        #     action_xmls[0] = action_xmls[0] + "\ninventory 1"
        # else:
        #     action_xmls[0] = action_xmls[0] + "\ninventory 0"
        # print(action_xmls)
        # exit()
        if not self._terminated:
            ...
            # print(f"step_message {step_message}")
            try:
        ...
        if n_instances_to_start > 0:
            ...
            with ThreadPoolExecutor(max_workers=n_instances_to_start) as tpe:
                for _ in range(n_instances_to_start):
                    ...
                    instance_futures.append(tpe.submit(self._robust_get_new_instance))
    ...
    def _robust_get_new_instance(self, port=None, instance_id=None, max_tries=10,
                                 ) -> InstanceManager:
